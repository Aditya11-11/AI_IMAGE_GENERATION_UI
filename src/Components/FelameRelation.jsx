...
const FemaleRelationship = () => {
  ...
  // Update localStorage when selectedRelationships or selectedFlavors change
  useEffect(() => {
    localStorage.setItem("relationships", selectedRelationships.join(", "));
  }, [selectedRelationships]);

  useEffect(() => {
    localStorage.setItem("personalityFlavors", selectedFlavors.join(", "));
  }, [selectedFlavors]);
  ...
  return (
    <div className="container text-center mt-5 text-white" style={{ paddingBottom: "20px" }}>
      ...
      <div className="d-flex justify-content-between mt-4">
        <Link to="/FemaleEyeHair" className="btn btn-outline-light px-5 py-2">
          ← Previous
        </Link>
        <Link to="/FemaleDetailsForm" className="btn btn-outline-light px-5 py-2">
          Next →
        </Link>
      </div>
    </div>
...
export default FemaleRelationship;
